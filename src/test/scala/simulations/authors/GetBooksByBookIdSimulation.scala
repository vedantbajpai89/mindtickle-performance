...
class GetBooksByBookIdSimulation extends Simulation{
  ...
  val authors = scenario("Get Books by BookId endpoint Tests")
    .exec(http("Get Books using Book Id")
      .get(getConfig.getBooksUsingId+"/"+getTestData.bookId)
      .headers(headers)
      .check(status is 200))
    .pause(getConfig.minimumTimeToThink second, getConfig.maximumTimeToThink seconds)// Add think time between requests
