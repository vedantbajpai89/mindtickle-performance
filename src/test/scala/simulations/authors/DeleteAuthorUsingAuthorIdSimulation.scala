...
class DeleteAuthorUsingAuthorIdSimulation extends Simulation{
  ...
  val authors = scenario("Delete Author by AuthorId endpoint Tests")
    .exec(http("Delete Author using Author Id")
      .delete(getConfig.deleteAuthorUsingId+"/"+getTestData.authorId)
      .headers(headers)
      .check(status is 200))
